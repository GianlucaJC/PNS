$(document).ready( function () {
    $('#tbl_list_gspr tfoot th').each(function () {
        var title = $(this).text();
		if (title.length!=0)
			$(this).html('<input type="text" placeholder="Search ' + title + '" />');
    });	
    var table=$('#tbl_list_gspr').DataTable({
		dom: 'Bfrtip',
		buttons: [
			'excel', 'pdf'
		],		
        ...
        language: {
            ...
            zeroRecords: 'Nessuna voce GSPR trovata',
            info: 'Pagina _PAGE_ di _PAGES_',
            infoEmpty: 'Non sono presenti voci per GSPR',
            infoFiltered: '(Filtrati da _MAX_ voci GSPR totali)',
        },
