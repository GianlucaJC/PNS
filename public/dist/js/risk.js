$(document).ready( function () {
    $('#tbl_list_risk tfoot th').each(function () {
        var title = $(this).text();
		if (title.length!=0)
			$(this).html('<input type="text" placeholder="Search ' + title + '" />');
    });	
    var table=$('#tbl_list_risk').DataTable({
		dom: 'Bfrtip',
		buttons: [
			'excel', 'pdf'
		],		
        ...
        language: {
            ...
            zeroRecords: 'Nessuna voce RISK trovata',
            info: 'Pagina _PAGE_ di _PAGES_',
            infoEmpty: 'Non sono presenti voci per RISK',
            infoFiltered: '(Filtrati da _MAX_ voci RISK totali)',
        },
